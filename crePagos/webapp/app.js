(function (angular) {
  ...
  myApp.service('crePagos', ['ethClinet', '$q', function (ethClinet, $q) {
    const crePagos = ethClinet.eth.contract(contractABI).at(contractAddress);
    ...
    this.getStatus = (paymentId) => $q((resolve, reject) => {
      return crePagos.getStatus.call(paymentId, callback(resolve, reject));
    });

    this.checkIfPaymentExists = (paymentId) => $q((resolve, reject) => {
      return crePagos.checkIfPaymentExists.call(paymentId, callback(resolve, reject));
    });

    this.getPrice = (paymentId) => $q((resolve, reject) => {
      return crePagos.getPrice.call(paymentId, callback(resolve, reject));
    });

    this.pay = (paymentId, price) => $q((resolve, reject) => {
      crePagos.pay(paymentId, {
        from: account,
        value: price
  ...
  myApp.controller('PaymentController', function ($scope, ethClinet, $timeout, EthService, crePagos, $routeParams, $q) {
    ...
    $scope.pay = () => {
      if ($scope.status == 'New') {
        ...
        crePagos.pay($scope.paymentId, ethClinet.toBigNumber($scope.price)).then(() => {
    ...
    $scope.getStatus = () => {
      return crePagos.getStatus($scope.paymentId).then(status => {
        $scope.status = status;
        return status;
      });
    };
    ...
    $scope.$on('$routeChangeSuccess', function () {
      ...
      $scope.getStatus().then(status => {
        if (status !== 'NotExists')
          return crePagos.getPrice($scope.paymentId);
      }).then(price => {
